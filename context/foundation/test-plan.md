...
| 8   | Session note silently dropped somewhere in the one-shot PATCH write path -- student types a note but it never reaches the dashboard                | Low    | Medium     | session-notes-and-chart plan (FR-014/FR-015); L-01 column-scope discipline re-applied to a new field spanning schema, hand-picked `.update()`, and dashboard select/render -- three points that must all agree                     |
| 9   | Focus-rating chart fails to render real rated-session data, or fails to degrade to the empty state below the 2-point threshold                     | Medium | Medium     | session-notes-and-chart plan (FR-016); roadmap S-04; chart is a `client:only="react"` island whose data crosses an SSR-fetch -> client-hydration boundary a jsdom test can't reproduce                                             |
...
| #7   | Picker mount with a failing fetch shows a visible load-error notice AND the energy buttons remain clickable (degraded but usable)                                                                                     | The TypeScript cast on `fetch().then()` looks safe -- `as Promise<{ topics: Topic[] }>` lies when the API returns the `{ error: string }` envelope on 401/500; the runtime read of `.topics` throws inside the `.then` and the floating `void` swallows it. Any future fetch site that copies this cast pattern recreates the bug. | Where the picker is mounted (currently `src/pages/session/new.astro`); the API error envelope shape; whether any sister component already has a `loadError` pattern to mirror | Vitest jsdom: `render(<EnergyPicker />)` with `vi.stubGlobal('fetch', ...)` returning a rejected promise or a non-ok Response; assert load-error UI                                                                                                                                             | Promoting to e2e because the failure feels user-facing -- a jsdom component test exercises the exact render path; e2e adds no signal here. |
| #8   | A rating-screen PATCH with a note results in that note appearing on the dashboard history card                                                                                                                       | "The schema accepts `note` therefore it round-trips" -- the schema, the hand-picked `.update()` write set, and the dashboard `.select()`/render are three independent points that must all agree                                                                                                                                 | Whether `note` is in `endSessionSchema`, in the PATCH `.update({...})` call, and in the dashboard query's select + `SessionListItem` type                                     | e2e (Playwright): full browser flow -- schema/API/UI/DB all in play, no single layer can prove the round-trip alone                                                                                                                                                                             | Unit-testing the schema's note validation alone and calling the risk covered -- it says nothing about whether the API writes it or the dashboard renders it                    |
| #9   | Dashboard shows the focus-rating chart once 2+ rated sessions exist, and the empty-state message below that threshold                                                                                                | "The component's `sessions.length < 2` branch is unit-tested therefore the dashboard integration is safe" -- the risk is whether SSR-fetched, filtered, and reversed data actually reaches and mounts the `client:only="react"` island                                                                                           | How `dashboard.astro` derives `ratedSessions` (filter + reverse of the existing query); that `FocusRatingChart` mounts via `client:only`, not `client:load`                    | e2e (Playwright): seed rated sessions via the service-role fixture, assert the real page transitions from empty state to chart across the hydration boundary                                                                                                                                   | Testing only the component in isolation (e.g. Vitest + Testing Library) -- proves the render logic but not that SSR data reaches the client island                            |
...
| 4   | E2e on full session capture flow             | Lock the user-visible success criterion (start → run → rate → history, including topic + material-format chips) as a regression gate before each future slice | cross-cutting | Playwright e2e                                         | complete | context/changes/testing-e2e-session-capture-flow/ |
...
### 6.3 Adding a test for a new RLS-bearing user-owned table endpoint

Any endpoint that writes to a user-owned, RLS-bearing table (e.g. `public.sessions`, `public.topics`, `public.material_formats`) must follow the column-scope discipline from L-01:
...
**System-seeded default rows.** Some user-owned tables ship rows with `owner_id IS NULL` that every authenticated user can SELECT but nobody can mutate (see `material_formats` migration `20260531182506`). Endpoints serving these tables must enforce a NULL-owner protection: PATCH or DELETE on a seeded row returns 409 (byte-identical with the cross-user-conflict shape). Reference test: `tests/integration/api/material-formats.update.test.ts` -- seeded-format-protection case.
...
- **Reference tests**:
  ...
  - `tests/e2e/session-note.spec.ts` -- note typed on the rating screen travels through the one-shot PATCH and appears on the dashboard history card (Risk #8).
  - `tests/e2e/focus-rating-chart.spec.ts` -- dashboard chart renders only once 2+ rated sessions exist, empty state below that threshold; seeds rated sessions via `insertSession` and reloads across the `client:only` hydration boundary (Risk #9).
