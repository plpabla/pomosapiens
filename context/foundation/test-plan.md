...
- **Reference test**: `tests/integration/api/sessions.end.test.ts` -- specifically the `"silently strips columns outside the contract (regression gate for L-01)"` test as the canonical template for column-scope regression gates.
...
- **Reference tests**:
  ...
  - PATCH column-scope: `tests/integration/api/sessions.end.test.ts` -- `"silently strips columns outside the contract (regression gate for L-01)"`.
