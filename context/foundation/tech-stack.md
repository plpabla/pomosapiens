...
**Updated 2026-05-26**: `deployment_target` changed from `cloudflare-pages` to `cloudflare-workers`. The current `@astrojs/cloudflare` adapter dropped Pages support, and the codebase already deploys via `npx wrangler deploy` (Workers), so the original Pages default was stale. Deploy with `wrangler deploy`, never `wrangler pages deploy` — the two are not interchangeable. See `context/changes/deployment/platform-research.md`.
